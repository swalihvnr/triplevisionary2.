...
export default function Testimonials() {
  return (
    ...
    >
      <div className="absolute top-1/4 right-0 w-[400px] h-[400px] rounded-full blur-[100px] pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(18,153,202,0.06), transparent)' }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          {feedback.map((item) => (
            ...
            >
              ...
              >
                <div className="flex items-center justify-between pb-4 mb-4" style={{ borderBottom: '1px solid rgba(26,51,68,0.06)' }}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-display font-bold text-xs leading-tight" style={{ color: '#1A3344' }}>{item.name}</h4>
                      <span className="text-[9px] block" style={{ color: '#6A8A9A' }}>{item.role}</span>
                    </div>
                  </div>
                  <Quote size={16} style={{ color: 'rgba(18,153,202,0.15)' }} />
                </div>

                <div className="flex gap-1 mb-4" style={{ color: '#1299CA' }}>
                  {Array.from({ length: item.rating }).map((_, sIdx) => (
                    <Star key={sIdx} size={12} fill="currentColor" />
                  ))}
                </div>

                <p className="text-xs leading-relaxed font-light italic" style={{ color: '#6A8A9A' }}>
                  "{item.review}"
                </p>

                <div className="mt-6 pt-3 flex items-center justify-between text-[9px] font-display" style={{ borderTop: '1px solid rgba(26,51,68,0.06)', color: '#6A8A9A' }}>
                  <span className="font-bold">Verified Review</span>
                  <span className="font-bold" style={{ color: '#1A3344' }}>{item.company}</span>
                </div>
              </GlassCard>
            </motion.div>
          ))}
